...
//	client2 "redis-sentinel/test/client"
//)
//
...
//func (f *Framework) WaitRedisclusterHealthy(name string, wait, timeout time.Duration) (result *rsv1.RedisSentinel, err error) {
//	// wait for redis cluster status change
//	time.Sleep(wait)
//	result = &rsv1.RedisSentinel{}
//	timer := time.NewTimer(timeout)
//	defer timer.Stop()
//	for {
//		select {
...
//			if result.Status.Conditions[0].Type == rsv1.ClusterConditionHealthy {
//				return
//			}
//			time.Sleep(time.Second * 5)
//		}
...
//func (f *Framework) CreateRedisCluster(rs *rsv1.RedisSentinel) *rsv1.RedisSentinel {
//	f.Logf("creating RedisCluster %s", rs.Name)
//	var err error
//	result := &rsv1.RedisSentinel{}
//	err = f.UtilClient.Get(context.TODO(), types.NamespacedName{
//		Namespace: f.namespace,
...
//		err = f.UtilClient.Create(context.TODO(), rs)
//		err = f.UtilClient.Get(context.TODO(), types.NamespacedName{
//			Namespace: f.namespace,
//			Name:      rs.Name,
//		}, result)
...
//func (f *Framework) UpdateRedisCluster(rs *rsv1.RedisSentinel) *rsv1.RedisSentinel {
//	f.Logf("updating RedisCluster %s", rs.Name)
//	err := f.UtilClient.Update(context.TODO(), rs)
//	gomega.Expect(err).NotTo(gomega.HaveOccurred())
//	return rs
//}
//
//// CreateRedisClusterAndWaitHealthy creates a RedisCluster and waiting for it to become Healthy
//func (f *Framework) CreateRedisClusterAndWaitHealthy(rs *rsv1.RedisSentinel, timeout time.Duration) *rsv1.RedisSentinel {
//	result := f.CreateRedisCluster(rs)
